...
namespace StrumentiMusicali.App
{
	public partial class MainView : UserControl, IMenu
	{
		...
		private void AggiungiImpostazioni()
		{
			...
			var ribProva = panel1.Add("Prova Backup Ftp", Properties.Resources.BackupDatabase);
			ribProva.Click += (s, e) => EseguiTestBackupEFtp();
		...
		/// <summary>
		/// Esegue il backup del database e l'upload su FTP, mostrando lo stato di avanzamento.
		/// </summary>
		private void EseguiTestBackupEFtp()
		{
			if (MessageBox.Show("Vuoi eseguire ora il backup del database e l'upload su FTP?",
				"Prova Backup Ftp", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
			{
				return;
			}
			...
			try
			{
				StrumentiMusicali.Core.Manager.ManagerLog.Logger.Info("Backup manuale inizio (test)");
				ProgressManager.Instance().Visible = true;
				ProgressManager.Instance().Messaggio = "Esecuzione backup database...";

				using (var uof = new StrumentiMusicali.Library.Repo.UnitOfWork())
				{
					uof.EseguiBackup();
				}

				ProgressManager.Instance().Messaggio = "Upload su FTP...";
				using (var back = new StrumentiMusicali.ftpBackup.Backup.BackupManager())
				{
					if (back.Manage())
					{
						StrumentiMusicali.Core.Manager.ManagerLog.Logger.Info("Backup Effettuato correttamente (test)");
						StrumentiMusicali.App.Core.MessageManager.NotificaInfo("Backup e upload su FTP eseguito correttamente");
					}
					else
					{
						StrumentiMusicali.Core.Manager.ManagerLog.Logger.Info("Errore durante il backup (test)");
						StrumentiMusicali.App.Core.MessageManager.NotificaInfo("Errore durante il backup e l'upload su FTP");
					}
				}
			}
			catch (Exception ex)
			{
				StrumentiMusicali.Core.Manager.ManagerLog.Logger.Error(ex, "Nella fase di backup manuale (test)");
				StrumentiMusicali.App.Core.MessageManager.NotificaError("Errore durante backup/upload FTP", ex);
			}
			finally
			{
