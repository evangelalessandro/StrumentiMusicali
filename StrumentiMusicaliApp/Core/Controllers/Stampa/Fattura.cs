...
namespace StrumentiMusicali.App.Core.Controllers.Stampa
{
  public class StampaFattura : IDisposable
  {
	...
	private void ImpostaQuadroIVa(List<FatturaRiga> righeFatt, Fattura fattura)
	{
	  ...
	  var rigaIniziale = 1;
	  if (fattura.TipoDocumento.GetAttrTipoFiscaleAttr().ScriviRigheETotaliStampa)
	  {
		//todo verificare se serve ivaapplicata valorizzata o meno, per ora lascio come prima
		foreach (var item in righeFatt.Where(a => a.Importo > 0).GroupBy(a => a.IvaApplicata).OrderBy(a => a.Key))
		{
		...
		imponibileIVaTot += +righeFatt.Where(a => a.Importo < 0).Sum(a => a.Importo);
	...
	private void ImpostaCampiTestata(Fattura fattura)
	{
	  ...
	  _excel.Range("TipoDocumento").Value = fattura.TipoDocumento.GetAttrTipoFiscaleAttr().Descrizione;
